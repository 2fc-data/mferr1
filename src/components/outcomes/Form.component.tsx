...
export const OutcomeForm: React.FC = () => {
  const { register, handleSubmit, formState: { errors } } = useForm<OutcomeFormData>();
  const [submissionStatus, setSubmissionStatus] = useState<string | null>(null);

  const onSubmit = async (data: OutcomeFormData) => {
    try {
      const response = await fetch(getURI(ENTITIES.OUTCOMES), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
      if (response.ok) setSubmissionStatus('Outcome created!');
  ...
  return (
    <div className="p-4 border rounded shadow-md">
      <h2 className="text-xl font-bold mb-4">Create Outcome</h2>
