...
export const CourtForm: React.FC = () => {
  const { register, handleSubmit, formState: { errors } } = useForm<CourtFormData>();
  const [submissionStatus, setSubmissionStatus] = useState<string | null>(null);

  const onSubmit = async (data: CourtFormData) => {
    try {
      const response = await fetch(getURI(ENTITIES.COURTS), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
      if (response.ok) setSubmissionStatus('Court created!');
  ...
  return (
    <div className="p-4 border rounded shadow-md">
      <h2 className="text-xl font-bold mb-4">Create Court</h2>
