...
interface CauseOutcomeHistoryFormData {
  cause_id: number;
  outcome_id: number;
  changed_by: number;
  change_date: string;
...
export const CauseOutcomeHistoryForm: React.FC = () => {
  const { register, handleSubmit } = useForm<CauseOutcomeHistoryFormData>();
  const [submissionStatus, setSubmissionStatus] = useState<string | null>(null);

  const onSubmit = async (data: CauseOutcomeHistoryFormData) => {
    try {
      const response = await fetch(getURI(ENTITIES.CAUSE_OUTCOME_HISTORY), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
      if (response.ok) setSubmissionStatus('Outcome History created!');
  ...
  return (
    <div className="p-4 border rounded shadow-md">
      <h2 className="text-xl font-bold mb-4">Create Outcome History</h2>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <Input type="number" id="outcome_id" {...register('outcome_id', { required: true, valueAsNumber: true })} />
        </div>
        <div>
